...
void Shapes::addArrow(Lines & lines, const vec3 & from, const vec3 & to, const vec3 & color)
{
	...
	float magnitude = length(to - from);

	vec3 edgeStart = to - (dir * (0.1f * magnitude));

	vec3 auxVector = dir.y != 1 ? vec3(0, 1, 0) : vec3(1, 0, 0);

	vec3 perp1 = normalise(cross(dir, auxVector)) * (0.05f * magnitude);
	vec3 perp2 = normalise(cross(dir, perp1)) * (0.05f * magnitude);

	vec3 edgeUp = edgeStart + perp1;
	vec3 edgeDown = edgeStart - perp1;
	vec3 edgeLeft = edgeStart + perp2;
	vec3 edgeRight = edgeStart - perp2;
